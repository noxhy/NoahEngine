...
func _ready() -> void:
	...
	album = load(Preload.character_data[GameManager.current_character].album_path)
	var background: Texture2D = load(Preload.character_data[GameManager.current_character].background_path)
